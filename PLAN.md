...
| 20 | 新增红果短剧（hongguoduanju）提取器 | ✅ 完成 | `go test ./...` 全绿；真机 `-j` 得 1 格式、下载后 ffprobe 校验 h264/aac 720×1280 | 新增 `extractor/hongguo`：匹配 `hongguoduanju.com/player/<series_id>`；抓 SSR 页面提取 `_ROUTER_DATA = {...}` JSON（whitespace-tolerant 括号匹配 `extractJSONAssign`）；从 `loaderData` 里动态定位含 `video_player_info` 的 page 条目，取 `main_url`（当前集 mp4 直链，腾讯云 COS）为唯一格式（h264/aac）；`seriesDetail` 取 series_name/series_intro/series_cover，`vid` 在 `vid_list` 中的位置定"第 N 集"拼入标题。**无需签名/cookie**，直连可用。当前仅单集（SSR 默认第 1 集），`?vid=` 不切集，切集需前端 episode API（未实现，待后续）。`core.go` 注册 + `main.go`/`README`/`PLAN` 更新；`hongguo_test.go`（URL 匹配/JSON 提取/解析/无视频报错） |
