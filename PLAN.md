...
| 21 | SponsorBlock 标记 + 章节解析 + embed-chapters | ✅ 完成 | `go test ./...` 全绿；真机 YouTube 视频 `--sponsorblock-mark` 得 4 段 sponsor、description 6 章节合并成 10 chapters | **SponsorBlock**：新增 `sponsorblock` 包——调 `https://sponsor.ajay.app/api/skipSegments/{sha256(videoID)[:4]}?service=YouTube&categories=[...]&actionTypes=["skip","poi","chapter"]`（**新隐私 API，用 hash 前 4 位而非裸 ID**，返回多 videoID 需过滤），解析 segments（过滤 (0,0) 整段）。`core.markSponsorblock` 在 processInfo 开头把段标成 `[SponsorBlock]: <cat>` 章节合并进 `info.Chapters`（日志走 stderr 避免污染 -j）。**章节解析**：`extractor.Chapter` + `Info.Chapters`；YouTube `parseChaptersFromDescription` 从 description 时间戳（`MM:SS`/`HH:MM:SS` 标题）解析，同时 `shortDescription` 填入 Description。**embed-chapters**：`postprocessor.FFmpegEmbedChapters` 生成 FFMETADATA 文件（TIMEBASE 1/1000、open-ended 章节补 END=duration、值转义）→ `ffmpeg -i in -i meta -map 0 -map_metadata 1 -c copy`。CLI 加 `--sponsorblock-mark`/`--sponsorblock-remove`(占位)/`--sponsorblock-categories`/`--embed-chapters`。注意：SponsorBlock remove（ffmpeg 切割）尚未实现，仅 mark。 |
