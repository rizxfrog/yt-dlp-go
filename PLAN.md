...
| 20 | 新增红果短剧原生提取器 + 整剧/切集 + fqnovel/CENC | ✅ 完成 | `go test ./...`；真实短链 `--playlist-items 1 -s` 验证 | 原 SSR-only 实现已升级：支持 novelquickapp 短链（仅跟首个重定向）、detail/player/`hongguo:<id>`；精确限定目标 `series_id` 的 `vid_list`；纯 Go 移植六神签名（Gorgon/Helios/Medusa）、类型码探测、fallback API、SPADE URL/内容密钥；`Format.DecryptionKey` 贯穿 core，ffmpeg 在其他后处理前原子 CENC 解密，并用首帧解码判定已解密文件以支持安全重跑。无 Python/GUOZI_SRC 依赖。 |
