...
| 16 | YouTube 改用 InnerTube 播放器 API + 子目录输出 bug 修复 | ✅ 完成 | 真实网络验证通过（本沙箱经代理可达 YouTube）；`go build/vet/test ./...` 全绿 + 新增 innertube_test.go | **修复用户报的 `error: no resolvable formats found`**：YouTube 观看页 `ytInitialPlayerResponse` 的 `adaptiveFormats` 现在只带元数据（itag/mimeType/length），不再带 `url` 或 `signatureCipher`，导致 30 个格式全被跳过。新增 `extractor/youtube/innertube.go` 走 InnerTube 播放器接口（`POST /youtubei/v1/player`），按 `visionos → android_vr → web` 顺序尝试 spoofed client；`visionos`/`android_vr` 返回明文直链、无需 JS 引擎。`extractor.go` 新增 `PostJSON`；`youtube.go` 重写为「先 InnerTube、后网页兜底」，并改进错误聚合（`summarizeSkips`）与 `-v` 日志。附：子目录输出 bug——`HTTPDownloader.Download` 直接 `os.OpenFile(part,...)` 未建父目录，导致带子目录的 `-o` 报 "The system cannot find the path specified"；已在 `downloader.go` 打开 `.part` 前 `os.MkdirAll(filepath.Dir(outPath))` 修复（DASH 路径本就用 `createFile` 已含 MkdirAll）。注意：用户提供的账号 cookie 已过期（YouTube 会轮换），但 InnerTube 公开直链路径无需 cookie，可忽略 "cookies no longer valid" 警告。新增 `innertube_test.go` 覆盖 visitorData 提取、countResolvable、formatsNeedJS、summarizeSkips、playabilityError、contextClient 固定值、collectRawFormats、客户端表完整性，以及用可注入 `playerAPIFn` 测试的 `resolvePlayerResponse` 回退/失败路径 |
...
| 20 | 新增红果短剧（hongguoduanju）提取器 + 整剧/切集 | ✅ 完成 | `go test ./...` 全绿；真机整剧下载 3 集全成功、ffprobe 校验 h264/aac 720×1280 | 新增 `extractor/hongguo`：匹配 `hongguoduanju.com/player/<series_id>`（整剧 playlist）与 `/player/<series_id>/<vid>`（单集）。抓 SSR 页面提取 `_ROUTER_DATA = {...}` JSON（whitespace-tolerant 括号匹配），从 `loaderData` 动态定位含 `video_player_info` 的条目；`main_url`（mp4 直链，腾讯云 COS）为唯一格式（h264/aac），`seriesDetail` 取 series_name/intro/cover。**切集 = 导航 `/player/<sid>/<vid>`**（loader key 变 `player_(series_id)/(vid)/page`，SSR 重渲染）。整剧 playlist：遍历 `vid_list` 并发（4）抓每集页面，**只抓前 `accessible_episode_cnt` 集**（未登录=3，付费集 404 跳过），`vid` 在 `vid_list` 位置拼「第 N 集」标题。**顺带修 downloader 把 403 改为 retryable**（红果 CDN 连续下载多集会返回瞬态 403 防盗链/限流，退避重试后成功；yt-dlp 对 CDN 也这样），加 `TestHTTPDownload_403Retryable`。注意：整剧批量请求会触发站点风控（`/player/<sid>` 一度 500），故限制并发+仅免费集；`?vid=` query 参数不切集（要用路径形式）。 |
| 21 | SponsorBlock 标记 + 章节解析 + embed-chapters + remove 切割 | ✅ 完成 | `go test ./...` 全绿；真机 `--sponsorblock-mark` 得 4 段 sponsor + 6 description 章节合并 10 chapters；`--sponsorblock-remove` 真机切割 90.98s→71.02s | **SponsorBlock**：新增 `sponsorblock` 包——调 `https://sponsor.ajay.app/api/skipSegments/{sha256(videoID)[:4]}?service=YouTube&categories=[...]&actionTypes=["skip","poi","chapter"]`（**新隐私 API，用 hash 前 4 位而非裸 ID**，返回多 videoID 需过滤），解析 segments（过滤 (0,0) 整段）。`core.fetchSponsorChapters` 拉段转 `[SponsorBlock]: <cat>` chapters（mark/remove 共用一次 fetch）；mark 合并进 `info.Chapters`（日志走 stderr 避免污染 -j）。**章节解析**：`extractor.Chapter` + `Info.Chapters`；YouTube `parseChaptersFromDescription` 从 description 时间戳解析，`shortDescription` 填 Description。**embed-chapters**：`FFmpegEmbedChapters` 生成 FFMETADATA（open-ended 补 END=duration）→ `ffmpeg -map_metadata`。**remove 切割**：`SponsorBlockRemove` 用 ffmpeg concat demuxer——`keepRanges`（镜像 yt-dlp `_make_concat_opts`：删除区间→保留区间 inpoint/outpoint）→ `ffconcat` spec → `ffmpeg -f concat -safe 0 -i spec -c copy out`；**concat 里 file 路径须绝对路径**（ffmpeg 相对 concat 文件解析）。`-c copy` 切割吸附关键帧（误差 ~0.04s），精确切割需重编码（未做）。CLI：`--sponsorblock-mark`/`--sponsorblock-remove`/`--sponsorblock-categories`/`--embed-chapters`。 |
| 22 | 元数据完善 + embed-metadata + write-thumbnail/description | ✅ 完成 | `go test ./...` 全绿；真机 Bilibili/YouTube dump 得统计字段，Bilibili 下载 `--embed-metadata --write-thumbnail --write-description` ffprobe 读回 title/artist/date/description/view/like/comment/repost | `extractor.Info` 新增 `Channel/ViewCount/LikeCount/CommentCount/RepostCount/Categories`；YouTube 填 `videoDetails.author/viewCount/keywords`，Bilibili 填 `stat.view/like/reply/share`，Douyin 填 `statistics.play_count/digg_count/comment_count/share_count`。`FFmpegMetadata` 扩展统计字段（view_count/like_count/comment_count/repost_count 写 `-metadata`），并**修复 output 临时名 `.meta.tmp` 让 ffmpeg 无法推断 muxer 的 bug**（改为保留扩展名 `v.meta.tmp.mkv`）。新增 `--embed-metadata`（core 后处理链在 embed-chapters 后）、`--write-thumbnail`（下载封面，Content-Type/URL 推断扩展名）、`--write-description`（写 `.description`）。 |
| 23 | Bilibili UGC 合集（season）整剧下载 | ✅ 完成 | `go test ./...` 全绿；真机 `-s` 识别 7 集 playlist、`--playlist-items 1-3` 下 3 集 ffprobe 校验时长逐一吻合 | 用户提供的 `BV1BYuo6JE1w` 实为 **UGC 合集**（多独立 BV 经 `season_id` 归集，非多P）。数据 **SSR 注入页面**：`videoData.ugc_season.sections[].episodes[]` 含每集 `bvid/cid/title/duration/arc`（cover/stat/author/pubdate），**无需额外接口**。`parseSeason` 解析出 `seasonInfo`；`Extract` 遇 `episodes>1` 走 `buildSeasonPlaylist`——每集构造完整 Info（arc 元数据）+ 各自 `cid` 调 playurl（WBI key 经 `fetchWbiKeys` 取一次复用）。**顺带修两个引擎级问题**：①`extractor.Format` 缺 `Headers` 字段 → 补上，`core.downloadFormat` 用 `mergeHeaders(AddHeaders, f.Headers)` 让 per-format 头生效；②B 站视频 CDN（`upos-sz-mirrorcos.bilivideo.com`）有 **Referer 防盗链**，无 Referer 时 DASH 视频流 403（音频流不拦）→ `extractPlayurlFormats` 给所有格式加 `Referer: https://www.bilibili.com/`。单视频路径不变。 |
